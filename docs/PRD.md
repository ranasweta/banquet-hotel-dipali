...
| 5 | Higher Authority (GM) | Approves escalations — menu exceptions, discounts beyond 10% of proposal, overdue wedding balances; visibility across all modules. Two accounts provisioned. |
...
- FR-1.7a **A part payment confirms (client's lead, amended 4 Aug 2026).** The 25% is no longer a gate on confirmation but a debt carried by it. A guest who brings less than 25% is confirmed all the same and blocks the venue windows and rooms like any other booking. "we cant let them go so what our team decided is lets take that money." Only an advance of **zero**, or one without a receipt number, is still refused — a date is held against money or not at all. The shortfall carries no deadline and raises no exception.
- FR-1.7b **A short booking is not marked (client, 8 Sep 2026),** withdrawing the "Downpayment due" surfacing of FR-1.7a. The calendar chip, its rose rule and legend entry, the shortfall and the guest's phone number on the day panel, the warning at the advance field and the toast after confirming are all removed: a booking confirmed on a part payment is drawn and read exactly like a paid-up one. Nothing about the money changes — the 25% and the wedding 50% are unaltered (FR-11.4, FR-11.5) and still reported by the milestones on the booking's Billing panel (FR-11.5a), which is where a shortfall belongs. `confirmEvent` continues to record what was owed at the moment the dates were held in the audit trail.
...
- FR-1.8b **The event type is correctable, by the Auditor, before confirmation (client, 8 Sep 2026).** A mis-picked type was unfixable — it was locked the moment the proposal existed, so the booking had to be abandoned and re-entered. It is now editable from the wizard's first step while the booking is an enquiry, and **only by the Auditor**: the type is the key every venue rate card is filed under (BR-R1), so changing it re-prices every function, which is the same authority he already holds over the venue, menu and lodge masters. The proposal total is recomputed in the same transaction; the food surcharge and the wedding 50% milestone (FR-11.5) follow on their own, being derived from the type on every read. The new type's contact rule (FR-1.11) is **not** applied to the correction — confirmation still enforces it, and blocking the correction itself would trap a booking mis-typed as an engagement in the wrong type permanently. **A confirmed booking is included (client's lead, same day),** overruling the enquiry-only limit this shipped with. A held function's venue rate is frozen from the old type's card, so the route re-cuts every one of them off the new card in the same transaction — otherwise the change is cosmetic and the hall goes on charging the old type's price. Two consequences follow and are accepted deliberately: a new type with **no rate card** for one of the halls refuses the change and names them (BR-R1; on a held booking there is no later gate, confirm having already happened), and the **25% advance can fall short** the moment it saves, since it was measured against the old total — nothing blocks on that, per BR-P1's debt-not-gate rule, and the audit row records both totals so the jump is traceable. It remains refused from **locked** onward for everyone: the guest holds a numbered document priced from those figures, and changing them is a supersede-and-reissue (FR-7.4, FR-6.2b), not a correction.
...
- FR-2.2b **An all-day live counter may share a hall with its own booking's functions.** *(Client, 13 Sep 2026, on two proposals that could not be confirmed.)* The hotel sells one thing that is not a sitting: a tea/coffee live counter charged flat for the day, which stands in the hall the party is already in while breakfast, lunch and dinner come and go around it. Entered as a function of its own — which is how it is priced — it collided with them under FR-2.2 and the booking was refused as though another guest had taken the slot. A menu tier may now be marked as **sharing** in the menu master (the Auditor's, `menu_tiers.shares_venue`), and a function on such a tier may overlap the OTHER FUNCTIONS OF ITS OWN BOOKING in one venue. **It reaches no further:** two different bookings still cannot hold one venue at overlapping times, sharing or not, and two ordinary functions of one booking still cannot either — that refusal now names the two functions instead of blaming a booking that does not exist. Enforced by two exclusion constraints on `venue_bookings` (migration 0038), with the flag snapshotted onto the hold when it is written.
...
- FR-4.7 **Withdrawn — a booking may take any number of rooms.** *(Client, 12 Sep 2026: "wherever the 35+ room trigger is just remove it, anyone can have any number of rooms, no 35 room warning or anything should be there".)* BR-L2's Higher Authority approval at 35 rooms is retired: no threshold, no exception raised, and no notice, chip or warning on any screen — the rooms panel, the lodging calendar and the Lodge Manager's home board are all silent on how many were taken. **The hard inventory cap stands** (FR-4.3, and CLAUDE.md rule 9): a party still cannot be given more of a category than the lodge physically has free on the tightest night of the stay, because that is a promise the hotel cannot keep. Migration 0037 settles every request that was pending when this landed — they gated confirm and the lock, they never held the rooms back, so settling one releases nothing — and drops the `large_allocation_rooms` setting. The `room_allocation_35plus` exception kind stays in the enum so decided rows still read on the approvals history.
...
- FR-6.1 A single approvals queue shall list all pending exceptions (menu increases beyond the free allowance, discounts beyond the 10% cap, overdue wedding balances, and any future exception types) with event context, requested change, requester, and age.
- FR-6.1a **Bundled by proposal (client's lead, amended 1 Aug 2026).** The queue is grouped by BOOKING, not by request: every pending ask on one proposal — menu increases from any function, an over-cap discount, and venue/date/time change requests (FR-1.9), which no longer have a queue of their own — is presented as a single item the Authority opens once. A request raised later joins the same bundle, whether or not part of that bundle has already been decided; nothing about *when* a request is raised changes, only how it is read and settled.
...
- FR-6.2c **The bundle screen, redesigned (client, 8 Sep 2026).** Four changes to how FR-6.2a is presented, none of them to what it decides.
  1. **The proposal collapses per function.** Each function, Rooms, and Prices & discounts is a section the Authority opens, closed by default, with the number of requests inside printed on its header. A booking with four functions used to open as several screens of dish checkboxes he had not asked to see.
  2. **No verdict buttons above the proposal.** Each request in the index at the top of the page carries one button that opens the section it is about and scrolls him to it — a menu increase to that function's menu, an over-cap price to the price sheet. He answers by editing what he finds there; his edits ARE the verdict, exactly as FR-6.2a already provided. There is no separate Decline: a dish unticked is an increase refused, and the actual price typed back into a cell is a discount refused. The services still take a rejection (FR-6.2); the screen no longer offers a verdict that can disagree with the booking under it.
  3. **A requested price is shown as a price.** An over-cap discount is held pending, so every reader shows the line at its actual price. The figure being asked for is now printed on the request, printed on the line beneath the price in force, and prefilled into the Authority's box — so approving as asked is the do-nothing answer, and typing over it the deliberate one. *"151000 is turned to 141000 in proposal but it's exceeding the 10% gap — hence it doesn't mean that 141000 should not be shown to the GM."*
  4. **One button.** The Discounted column travels with the decision and commits in the same transaction (FR-6.2a), instead of saving on its own.
- FR-6.2d **The Authority is shown the whole discount, as a percentage (client, 8 Sep 2026):** *"it should also show the current total % discounted from total so that GM can get the idea how much is discounted on that event."* The price sheet, for him alone, states the bill at list price, what is already in force, what is being requested, what the column he is typing comes to, and the cap — each in rupees and as a share of the bill. It is his screen only: the same standing figures beside a guest at the counter tell them there is a bigger discount to push for, which is why FR-11.1a's headroom notice is announced only when it is crossed.
...
- FR-11.3 **Overall cap (BR-D2):** the combined discount across all heads must remain ≤ 10% of the total bill — venue + food + rooms, pre-tax (amended 25 Jul 2026; was venue+food only). **No GST enters that base — neither band of the room tax, nor the 18%** (FR-11.7, FR-11.7a). Exceeding the cap auto-raises a Higher Authority exception. **It never blocks the confirm** (amended 12 Sep 2026): a discount given within the cap can drift over it when the bill moves underneath, and confirm used to refuse — stranding a manager who was holding the guest's advance with no way to reach the Authority from that screen. The booking confirms, the advance is recorded and the dates are held; the over-cap figure goes to the queue as one request, and the discount stays in force until the Authority decides, since a confirmation is the one moment a quoted price must not move. Per-room caps (BR-D1, FR-11.2) are retired now that rooms are booked in bulk.
- FR-11.3c **The base is priced the way the bill is (client, 8 Sep 2026: *"the cap calculation is also coming wrong"*).** BR-D2's base is measured through the same pricing service the proposal and the bill use, not by a second copy of the arithmetic. Two things it was getting wrong: a hall hired for a day and used for three functions was counted three times, though the day's earliest function carries the whole let (BR-C1, rule 3), which inflated an enquiry's 10% by two hall charges; and the Chef's priced delicacies and the bar were absent, though both are inside `proposal_total_paise`. The two branches of the test — the stored total for a confirmed booking, the live figure for an enquiry — therefore measured different bills, and the cap moved at confirmation without a price changing. They agree now, line for line.
- FR-11.3d **A request is superseded whole (8 Sep 2026).** One save over the cap raises ONE exception covering every cell in it (FR-11.3, and the "one decision per save" rule). Re-pricing any one of those cells retires the request and every cell of it — none of which was ever in force — rather than leaving part of a superseded ask waiting in the queue to overwrite the price that replaced it. A request already **decided** is left alone: its rows are in force on lines the new save never mentioned.
- FR-11.3e **A requested line is always decided by the Authority's save (8 Sep 2026),** and the record says what was asked. Because a pending discount is not applied, the price in force on a requested line IS its actual — so refusing the ask by typing the actual back (FR-6.2c) changes nothing measurable, and a save that carried only changed cells would send nothing and let the ask be approved by default: the opposite of what he typed. His save therefore carries every requested cell whatever he did with it. The audit row for one reads from the ASKED-FOR price rather than the price in force — "₹20,000 requested → ₹1,51,000 of ₹1,51,000" — so a refusal, which moves no money, still leaves a record of itself. A counter's save is unaffected: their boxes are prefilled with the price in force, and re-sending a colleague's pending line would cancel the request they are waiting on.
