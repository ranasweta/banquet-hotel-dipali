...
- FR-1.7 Confirmation shall require guest name, contact number(s) per FR-1.11, Aadhaar images on file (FR-1.10), and a recorded down payment of 25% of the proposal value with an internal receipt number (BR-P1); only this recorded payment atomically blocks all venue-day slots of every sub-event. If any slot was taken between validation and confirm, the confirm fails with a clear message.
...
- FR-2.2 **Venue time-overlap (BR-C1, amended 17 Jul 2026).** A new booking may occupy a venue only if its time window does not overlap any existing booking on that venue. A venue may therefore host several sub-events on the same day (e.g. a morning haldi and an evening sangeet) provided their windows don't overlap. **Back-to-back is permitted** — a window ending at 15:00 and another starting at 15:00 do not conflict. A window whose end time is at or before its start time runs past midnight into the next day, and blocks the next morning accordingly. The fixed carryover/main-slot model and the 11 AM handover are withdrawn. *(Client-confirmed: "simple logic — if time doesn't overlap, booking accepted"; the booking form captures start and end time.)*
...
- FR-2.5 **The calendar carries locked-in deals only.** *(Amended 17 Jul 2026.)* Only events that have reached Confirmed or beyond (Confirmed, In Progress, Completed, Locked, Billed, Closed) appear on the board. Enquiries are never rendered, and the enquiry-contested (dashed, with count) state is withdrawn. The calendar shall visually distinguish: confirmed, in-progress, and carryover (the next-morning tail of a sub-event that ran past midnight) states.
...
- FR-2.6 Pax entered for a sub-event shall be validated against the venue's capacity range; out-of-range values warn (soft block) and require an explicit override note.
...
- FR-11.1 Discounts may be given at any head — per-plate menu rate, venue charge, per-room rate — or as an overall adjustment, each with a mandatory remark; the system computes the combined discount live against the proposal total.
...
- FR-11.3 **Overall cap (BR-D2):** the combined discount across all heads must remain ≤ 10% of the total proposal value. Exceeding the 10% cap or any per-room cap auto-raises a Higher Authority exception; the discounted proposal cannot be confirmed until approved.
- FR-11.3a **The Authority's own discount (client's lead, amended 1 Aug 2026).** A discount entered by the Higher Authority himself, from the approvals screen, is exempt from BR-D2's cap: it is recorded with no linked exception and takes effect immediately. The cap exists to route a large discount *to* him, and there is no one at the other end of that round trip when he is the one giving it. He may give it as a flat rupee amount (stored fixed) or as a percentage of a head (recomputed live, per FR-11.1); either way it flows through the bill, the balance, the advance base and every other reader of the effective discount. The mandatory remark is not waived.
- FR-11.4 **Date-block payment (BR-P1):** venue dates block only when a down payment of 25% of the proposal value is recorded; the confirm screen prompts the Booking Manager to attest that payment was received and to enter the internal receipt number.
- FR-11.5 **Wedding balance schedule (BR-P2):** for weddings, the remaining balance is due 30 days before the first event date. The system notifies the Booking Manager at D-30 and reminds through D-21; if still unpaid from D-20 onward, reminders are also sent to Higher Authority. (Interpretation to confirm — see open question 1.)
...
1. Payment reminders: we have assumed Booking Manager reminders run D-30 to D-21 and Higher Authority is copied from D-20 onward for unpaid wedding balances — please confirm this schedule, and whether the 25% block payment also applies to non-wedding events.
2. Cancellation: refund/retention rules by notice period, and who may execute a cancellation?
...
4. Should the GM approval rule also govern pax increases beyond venue capacity, or only menu items?
5. Tax treatment confirmation: GST rates to apply per charge head (venue rental, food per plate, rooms by tariff slab, maintenance recoveries) — to be finalised with the hotel's tax consultant.
