...
- FR-2.2 A new booking may occupy a venue-day only if its main slot is free; if a carryover exists that morning, the new sub-event's start time must be ≥ 11:00 AM, and the outgoing sub-event's end time must be ≤ 11:00 AM (BR-C1).
- FR-2.3 Booking a venue bundle shall block every member venue's slots; booking any member venue shall block the bundle.
...
- FR-2.5 **The calendar carries locked-in deals only.** *(Amended 17 Jul 2026.)* Only events that have reached Confirmed or beyond (Confirmed, In Progress, Completed, Locked, Billed, Closed) appear on the board. Enquiries are never rendered, and the enquiry-contested (dashed, with count) state is withdrawn. The calendar shall visually distinguish: confirmed, carryover-till-11-AM, and in-progress states.
