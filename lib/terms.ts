 ...
 *
 * The annexure is a FACSIMILE of that PDF, not a redesign: same boxed page, same centred
 * masthead, same bold "N. Title" headings each followed by its bullet(s), same two-page split,
 * same CLIENT ACCEPTANCE block, same "Client Signature / Authorised Signatory / Page N of 2"
 * footer. Clauses therefore carry `bullets: string[]` — clause 9 has three in the PDF and
 * collapsing them into one paragraph is exactly the kind of drift this shape prevents.
 */
...
export type TermClause = { title: string; bullets: string[] }
...
export const TERMS_CLAUSES: TermClause[] = [
  ...
  {
    // Three bullets in the client's PDF, and three here. Merging them reads as a rewrite.
    title: 'Decoration and Vendors',
    bullets: [
      'Decoration, lighting, stage, sound, LED, DJ and entertainment shall be charged separately unless included in the quotation. Outside vendors require prior approval and payment of applicable maintenance charges.',
      'Nailing, drilling, stapling, pasting or damaging any kind of hotel property is prohibited. Fireworks, inflammable material, open flames, smoke machines, confetti and colour powder shall not be used without approval.',
      'The Client shall be responsible for damage caused by guests, vendors, decorators or invitees.',
    ],
  },
  {
...
/**
 * The PDF breaks after clause 12 (Parking); 13–25 and the acceptance block are page 2 of 2.
 * The replica breaks in the same place so the annexure is two pages, exactly as supplied.
 */
export const TERMS_PAGE_1_CLAUSES = 12

export const TERMS_ACCEPTANCE_TITLE = 'CLIENT ACCEPTANCE'
...
/** The acceptance fields, with the PDF's own dotted-rule filler as the value placeholder. */
export const TERMS_ACCEPTANCE_FIELDS: { label: string; value: string }[] = [
  { label: 'Name of Client/Organisation', value: '……………………………………………………………' },
  { label: 'Event Name/Type', value: '……………………………………………………………' },
  { label: 'Event Date', value: '…..…/…../…….. To ……/……/………' },
  { label: 'Date', value: '…../…../………' },
  { label: 'Place', value: 'Hotel Dipali, Sagar' },
]
