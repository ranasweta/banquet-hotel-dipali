...
async function list(qs: string): Promise<{ events: Row[]; eventTypes: { code: string; displayName: string }[] }> {
  sessionState.userId = bm.id
  const res = await listEvents(new NextRequest(`http://test/api/v1/events${qs}`))
  return (await res.json()) as { events: Row[]; eventTypes: { code: string; displayName: string }[] }
}
...
d('searching the proposals list', () => {
  ...
  /**
   * The dropdown offers the TWO types a booking can be made as, not the six rows the table
   * carries (client, 22 Aug 2026). Engagement, mahila sangeet, birthday and corporate are
   * unreachable — the wizard has never offered them and no booking in the database is one — so
   * as filter options they could only ever return nothing. The pair comes from the same
   * `BOOKABLE_EVENT_TYPES` the wizard and the venue master read.
   */
  it('offers only the two types a proposal can be made as', async () => {
    const { eventTypes } = await list('')
    expect(eventTypes.map((t) => t.code).sort()).toEqual(['other', 'wedding'])
    // "Others" is the word the hotel uses; the table says "Other".
    expect(eventTypes.find((t) => t.code === 'other')!.displayName).toBe('Others')
  }, 120_000)
